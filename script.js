...
async function updateStatus() {
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('username').textContent = data.data.discord_user.display_name ? data.data.discord_user.display_name : data.data.discord_user.username;
            ...
            document.documentElement.style.setProperty('--status-color', statusColors[status]);
            ...
            if (data.data.listening_to_spotify) {
                ...
                document.getElementById('spotifySong').textContent = data.data.spotify.song;
                document.getElementById('spotifyArtist').textContent = data.data.spotify.artist;
            } else {
...
const updatePerformanceMode = () => {
    ...
    if (container) {
        container.style.filter = isPerformanceMode ? 'none' : 'blur(0.5px) brightness(1.1)';
    }

    if (isPerformanceMode) {
        document.getElementById('container').style.animation = 'none';
    } else {
        document.getElementById('container').style.animation = 'chromaticMove 50ms infinite alternate';
    }
...
class CommentsUI {
    ...
    async fetchInitialData() {
        try {
            ...
            if (data.rateLimit?.isLimited) {
                this.updateRateLimitState(data.rateLimit || { isLimited: false, remainingMinutes: 0 });
            }
        } catch (error) {
    ...
    updateButtonText(minutes) {
        ...
        this.submitButton.textContent = minutes === 1
            ? '> WAIT 1 MINUTE'
